...
- **Nothing but the seed script writes to `content.fetch`.** `scripts/seed_fetch.py` requires `--redis-url` and `--topic` explicitly and additionally requires `--production` for the one combination the live worker consumes (db 0 **and** `content.fetch`) — a frame there is fetched for real. Its `--watch` reads `content.blobs` with a plain `XREAD` and never joins a group: a group left by an operator tool accumulates a PEL nothing drains.
