...
- `REPLICATOR_REPLICATION_ALIASES_FILE` — path to the JSON alias table; **unset by default, and that is the safe posture**. Nothing provisioned means every `content.replicate` command is refused with `alias_unknown`, so enabling replication to a destination is an explicit operator act on the VM rather than a consequence of a message arriving (contract T5 — it matters most for `ia`, whose items cannot be deleted). A *path* rather than the table itself because the provisioned set is host state but is a table, and one variable per alias per field is a shape env does not hold. Format:
  ...
  ```json
  { "primary": { "provider": "gcs", "bucket": "co-gcs-replication", "prefix": "organizations" } }
  ```

  **`prefix` is the containment root, and picking it is a security decision, not cosmetics.** The bucket has two top-level prefixes — `organizations/` (what replication writes) and `console_workspace/` (what it must not). Binding the alias to `organizations` means a rendered destination can never reach the other, and the issuer renders *relative* to it, so the final object keys are byte-identical to what the CLI writes today. Leaving `prefix` empty would produce the same keys and no containment at all: T3's root would be the whole bucket.
