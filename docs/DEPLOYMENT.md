...
- **`idempotency_key` is `<unit>:$MONITOR_INVOCATION_ID`**, the failed run's InvocationID, which systemd ≥251 hands an `OnFailure=` handler (co-replicator runs 255). It is null when systemd supplied none. **Never reuse a key to retry:** notifier answers a replayed key with the *prior* record, `failed` included, and makes no new delivery attempt. The handler makes one POST per failure and no retries.
