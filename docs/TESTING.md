...
**Where fakeredis diverges.** It is sound for consumer-group *mechanics* — what state a command leaves behind — but diverges on *lifecycle* and *blocking* semantics: it registers a consumer on an empty `XREADGROUP` (real Redis waits for a delivery, GH #3) and it ignores `block` (worked around by `IDLE_SLEEP_SECONDS` in `src/worker/loop.py`). Its `XAUTOCLAIM` is wrong outright (fakeredis 2.37.0): the cursor is the highest *claimed* id rather than the next to scan, never `0-0`, with no `count * 10` attempt budget — so a cursor-following walk on it re-claims the entry it just claimed and never ends. `fake_redis` replaces the command with one rebuilt from `XPENDING` + `XCLAIM`, which the fake does get right, and `tests/worker/test_fake_xautoclaim_integration.py` holds the rebuild to a real server scenario by scenario (#109). A test that patches `fake_redis.xpending_range` to simulate a refusal still refuses only the worker's own call: the rebuild binds its primitives at fixture creation. Rule of thumb: an assertion about **what state results** is safe against the fake; an assertion about **when Redis does something** needs a live broker. Both divergences were found by running against the real server, not by the suite.
