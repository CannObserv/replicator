...
12. Old host: `sudo systemctl disable --now replicator && sudo systemctl mask replicator`.
    Confirm `worker stopped` in the journal.
...
18. **Rollback**, until Phase 7: new host `disable --now`, then old host `unmask && enable --now`.
    The consumer names are the same, so the PEL follows. Triggers:
...
24. `docs/SOCRATICODE.md`: one line saying this host has no install until notifier#57.
...
27. Unit: `unmask`, remove `/etc/systemd/system/replicator.service`, `daemon-reload`,
    `reset-failed`.
