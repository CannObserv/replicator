...
- **`claim_stale` is the retry path, not just crash recovery.** A transiently-failed message is left unacked and comes back through the same reclaim, so retry cadence = `REPLICATOR_CLAIM_MIN_IDLE_MS`. Call it with `count=1`: XAUTOCLAIM transfers ownership and resets the idle clock on every entry it returns *before* co-core decodes them, and it restarts at `0-0` each call, so a poison entry jams recovery permanently unless it is DLQ'd first. **Recovery runs first, never twice running (#98):** `XAUTOCLAIM` restarts an entry's idle clock at the *claim*, so a handler slower than the window that fails transiently releases an entry that is reclaimable at once — claimed ahead of every read, it stopped the group reading while the worker logged a retry per cycle. So a reclaim owes the stream a turn: the next poll takes a non-blocking look there first (`PollCadence` in `src/worker/loop.py`), and at most one reclaim runs between two looks — the head of the stream waits for the handler in flight plus at most one reclaim, however slow either is. Retry cadence is therefore the window or one turn of the stream, whichever is longer.
