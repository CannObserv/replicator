...
- **DLQ is a shipped seam, not a TODO:** `dead_letter(message_id, fields)` copies the frame to `<topic>.dlq` and acks the original. Deterministic failure ⇒ DLQ; transient failure ⇒ retry. **Draining it is this service's job, and both halves are granted** — `XADD <topic>.dlq` parks a frame and, since 2026-09-11, an ACL selector `(+xdel ~content.fetch.dlq ~content.replicate.dlq)` removes one once it is triaged (#86, broker#12). `XDEL` is refused on every other stream and there is no `XTRIM`, so disposal is one entry at a time by id and a drain can never become a wipe. Resting state for both queues is 0: `docs/COMMANDS.md`.
...
- **`claim_stale` is the retry path, not just crash recovery.** A transiently-failed message is left unacked and comes back through the same reclaim, so retry cadence is at least `REPLICATOR_CLAIM_MIN_IDLE_MS`. Call it with `count=1`: XAUTOCLAIM transfers ownership and resets the idle clock on every entry it returns *before* co-core decodes them, and a claim that raises on a poison entry returns nothing else, so each one is DLQ'd before the next claim. **Recovery walks the PEL; it does not restart at `0-0` (#102).** Among several slow transient failures the oldest is reclaimable at every turn, so from the head it takes all of them and a younger entry is delivered once and never again. `PollCadence.reclaim_from` carries the claim's position across cycles: each claim starts just past the last entry returned and wraps to the head when nothing lies past it — computed, because co-core's `claim_stale` discards `XAUTOCLAIM`'s cursor. Not persisted; a restart starts at the head. What it does not lift is `XAUTOCLAIM`'s scan of at most ten entries per `count=1` call, which delays an entry behind ten young ones by at most one window. **Recovery first, never twice running (#98):** a reclaim owes the stream a non-blocking look before the next one — `XAUTOCLAIM` restarts the idle clock at the claim, so a slow transient failure is otherwise reclaimable ahead of every read, forever. The head of the stream waits for the handler in flight plus at most one reclaim. Why: `PollCadence` in `src/worker/loop.py`.
- **Retry accounting is XPENDING's `times_delivered`**, not a side counter. It only advances on a reclaim. **The broker has granted it only since 2026-09-22 (broker#39)**, and until then the ceiling never fired (#103). **A count that cannot be read retries the message; it never escapes it.** The read runs inside `process_message`'s `except Exception` arm, where the sibling transient arm cannot catch it, so before #103 a refused `XPENDING` left as a *cycle* failure: `run_loop` blamed the broker and backed off, the handler's error survived only as chained context, and a handler failing every command climbed `REPLICATOR_MAX_CONSECUTIVE_CYCLE_FAILURES` into a restart loop. Now it is one ERROR per attempt naming both errors (`count_error` beside `error`), and `Outcome.RETRY`, for #82's reason: an unreadable count says nothing about the command, and closing it would be a terminal `handler_error` for what may be its first failure. `tests/worker/test_oom_integration.py` drives the ceiling through a copy of the production grant, and the same grant without `+xpending`.
