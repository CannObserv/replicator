...
- **A frame that fails to decode has no fields — on the read path.** `from_wire` raises from *inside* `read`, so the anomaly carries `topic` + `message_id` only — but `dead_letter` XADDs the fields it is given and `XADD` rejects an empty map. Re-read the raw frame by id (`XRANGE topic id id`) and fall back to a synthesized record when the entry has been trimmed. `src/worker/loop.py::dead_letter_anomaly`. The claim path has neither problem: `claim_stale_page` returns each undecodable entry as a `PoisonFrame` carrying its raw fields (#109), routed by `dead_letter_poison`.
...
- **`claim_stale` is the retry path, not just crash recovery.** A transiently-failed message is left unacked and comes back through the same reclaim, so retry cadence is at least `REPLICATOR_CLAIM_MIN_IDLE_MS`. It goes through `claim_stale_page` (cannobserv#465, co-core 0.19), which returns poison frames rather than raising on them, so a bad entry no longer strands the ones claimed beside it. Still `count=1`, now for the #98 bound below: a batch would hold the stream back one handler per entry claimed. **Recovery walks the PEL; it does not restart at `0-0` (#102).** Among several slow transient failures the oldest is reclaimable at every turn, so from the head it takes all of them and a younger entry is delivered once and never again. `PollCadence.reclaim_from` carries the claim's position across cycles: each claim starts just past the last entry returned — `(<id>`, the exclusive start, resolved by the server — and wraps to the head when the scan reports the end. **The end is `XAUTOCLAIM`'s `0-0` cursor, not an empty page (#109).** One `count=1` call inspects at most ten pending entries, so an empty page with a live cursor is scanned on from, up to `MAX_CLAIM_PAGES` a turn, and the next turn resumes at that cursor. Not persisted; a restart starts at the head. **A pending entry trimmed from the stream** is dropped from the PEL by `XAUTOCLAIM` itself and reported in the page's `deleted`; the worker logs it as a WARNING (`pending entries were trimmed`), the only record the command was never handled. **Recovery first, never twice running (#98):** a reclaim owes the stream a non-blocking look before the next one — `XAUTOCLAIM` restarts the idle clock at the claim, so a slow transient failure is otherwise reclaimable ahead of every read, forever. The head of the stream waits for the handler in flight plus at most one reclaim. Why: `PollCadence` in `src/worker/loop.py`.
