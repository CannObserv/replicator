...
- `REPLICATOR_PERMANENT_BUCKET` — the permanent content-addressed store (#114); **no default**, and empty reads none. Not a temp store and never written by the fetch path: it is a second **source** for `content.replicate`, whose `blob_uri` may name `gs://<bucket>/blobs/<sha256>.bin`, so a publication can follow a persist long after the temp blob expired. Read under either backend, because persisted bytes live in GCS whichever backend holds the temp tier. Built with touch **off** — the permanent writer holds no `update`, and a permanent object has no clock to move — and preflighted at boot like the temp bucket: a bucket that does not answer stops the boot, naming it (`permanent blob bucket is not usable`); a good one logs `reading persisted blobs from a permanent store`. The prefix is the store's default, `blobs`, not a setting: readers derive the location with co-core's `gcs_uri(bucket, digest)`, whose default it also is. `co-gcs-replicator` on this VM from the #114 step 5 deploy ([INFRASTRUCTURE.md](INFRASTRUCTURE.md))
